...
class VolatilityDashBoard:
    def __init__(self, root):
        self.root = root
        self.root.title('Implied Volatility Dashboard')
    ...
    def create_equity_contract(self, ticker):
        contract = Contract()
    ...
    def setup_ui(self):
        ...
        plot_frame = ttk.LabelFrame(main_frame, text='Implied Volatility Results', padding='5')
    ...
    def query_data(self):
        ...
        if 1 in self.ib_app.historical_data:
            ...
            if len(data) > 0:
                ...
                self.log_message('Note: IB iVol values are already annualized')
    ...
    def process_implied_volatility(self):
        ...
        # FIXED: Don't multiply by sqrt(252) - IB data is already annualized!
    ...
    def update_vol_display(self):
        if self.current_implied_vol is not None:
            self.current_vol_label.config(text=f'{self.current_implied_vol*100:.2f}%')
            self.vol_computation_label.config(text='Already annualized by IB')
            ...
            if self.equity_data is not None:
                ...
                # FIXED: Format as percentages for consistency
                range_text = f'Min: {vol_min*100:.1f}% | Mean: {vol_mean*100:.1f}% | Max: {vol_max*100:.1f}%'
            else:
                range_text = 'N/A'
            ...
            if self.current_implied_vol > 0.4:
                ...
                self.current_vol_label.config(foreground='blue')
    ...
    def analyze_volatility(self):
        ...
        vol_forward_30 = self.volatility_data['implied_vol'].rolling(window=1).mean().shift(-30)
        
        analysis_df = pd.DataFrame({
            'current_vol': self.volatility_data['implied_vol'],
            'forward_30_vol': vol_forward_30,
            'vol_diff': vol_forward_30 - self.volatility_data['implied_vol'],
        ...
        slope1, intercept1, r_value1, p_value1, std_error1 = stats.linregress(
            analysis_df['current_vol'],
            analysis_df['forward_30_vol']
        )
        
        slope2, intercept2, r_value2, p_value2, std_error2 = stats.linregress(
            analysis_df['current_vol'],
            analysis_df['vol_diff']
        )
        ...
        self.ax1.scatter(analysis_df['current_vol'], analysis_df['forward_30_vol'], alpha=0.6, s=20)
        ...
        min_val = min(analysis_df['current_vol'].min(), analysis_df['forward_30_vol'].min())
        max_val = max(analysis_df['current_vol'].max(), analysis_df['forward_30_vol'].max())
        ...
        # Plot 2: Vol Difference with Regime Analysis
        ...
        self.ax3.axhline(y=self.volatility_data['implied_vol'].mean(), color='black', 
        ...
        self.log_message('Volatility analysis complete')
